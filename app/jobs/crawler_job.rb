    ...
    if crawl.html.index("Page Not Found").present?
      ...
      crawl.data.each do |n|
        ...
        if image.class == String
          download     = open(image)
          number       = rand(100)
          IO.copy_stream(download,Rails.root.to_s + "/tmp/images/#{instagram_id}#{number}.png")
          url_img = open(Rails.root.to_s + "/tmp/images/#{instagram_id}#{number}.png")
          ...
          image.each do |image_url|
            ...
            number       = rand(100)
            download = open(video)
            IO.copy_stream(download,Rails.root.to_s + "/tmp/videos/#{instagram_id}#{number}.mp4")
            url_video = open(Rails.root.to_s + "/tmp/videos/#{instagram_id}#{number}.mp4")
      ...
      crawl.data_user.each do |users|
        ...
        IO.copy_stream(download,Rails.root.to_s + "/tmp/avt/avatar.png")
        url_avt = File.open(Rails.root.to_s + "/tmp/avt/avatar.png")
